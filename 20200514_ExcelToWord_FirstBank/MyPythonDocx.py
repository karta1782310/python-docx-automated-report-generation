    ...
    def set_content_font(table, size=Pt(14), bold=False, align=WD_ALIGN_PARAGRAPH.CENTER, name=u'微軟正黑體'):
        for i, row in enumerate(table.rows):
            ...
            for cell in row.cells: 
                ...
                Parag.set_font(cell.paragraphs, size, tmp_bold, align, name)
...
class Parag():
    ...
    def set_run_font(paragraph, size=14):
        for run in paragraph.runs:
            run.font.size = Pt(size)                        
            run.font.name = 'Times New Roman' # 設置英文字體
            run._element.rPr.rFonts.set(qn('w:eastAsia'), u'標楷體') # 設置中文字體
