...
class Ui_Test(object):
    def setupUi(self, Test):
        ...
        self.label_0.setMinimumSize(QtCore.QSize(80, 0))
        self.label_0.setMaximumSize(QtCore.QSize(80, 16777215))
    ...
    def clickedconnect(self):
        self.btn_word.clicked.connect(lambda: self.open_word(self.word_in))
        self.btn_excel.clicked.connect(lambda: self.open_excel(self.first_in))
        self.btn_save.clicked.connect(lambda: self.open_path(self.save_in))
        self.btn_run.clicked.connect(self.first_run)

        self.btn_excel_2.clicked.connect(lambda: self.open_excel(self.first_in_2))
        self.btn_excel_3.clicked.connect(lambda: self.open_excel(self.second_in))
        self.btn_save_2.clicked.connect(lambda: self.open_path(self.save_in_2))
    ...
    def open_word(self, obj):
        file_name, _ = QFileDialog.getOpenFileName(self, "選取檔案", "./", "Word Files (*.docx)")
    ...
    def first_run(self):
        ...
        if not word or not company or not cover_date or not tool_name or not tool_version:
            ...
            try:
                ...
                errMsg = "File \"{}\", line {}, in {}: [{}] {}".format(fileName, lineNum, funcName, error_class, str(e))

                self.textEdit.append("錯誤:")
                self.textEdit.append(errMsg)
    ...
    def second_run(self):
        ...
        if not excel or not sheet or not excel2 or not sheet2 or not consultant2 or not second_date or not save:
            ...
            try:
                ...
                self.textEdit_2.append("錯誤:")
                self.textEdit_2.append(errMsg)
