...
class Table():
    ...
    def set_content_font(table, size=Pt(14), bold=False, align=WD_ALIGN_PARAGRAPH.CENTER, name=u'微軟正黑體'):
...
class Parag():
    ...
    def set_font(parags, size=Pt(14), bold=False, align=WD_ALIGN_PARAGRAPH.CENTER, name=u'微軟正黑體'):
        ...
        if isinstance(parags, list):
            for paragraph in parags: 
                ...
                for run in paragraph.runs:
                    run.bold = bold 
                    run.font.size = size                        
                    run.font.name = 'Times New Roman' # 設置英文字體
...
def plt_bar(title, names, values, picture):
    ...
    bar = plt.bar(names, values, color=['indianred', 'cornflowerblue', 'yellowgreen'])
    for rect in bar:
        h = rect.get_height()
...
def risk_bar(doc, wb, sheet, ips=[]):
    ...
    for table in doc.tables:
        if "riskBar" in table.cell(0, 0).paragraphs[0].text:
            ...
            # add pic in table
            plt_bar('風險', names, values, "risk_bar.jpg")

            main_cell = table.cell(0, 0)
            Parag.delete(main_cell.paragraphs[0])

            parag = main_cell.add_paragraph()
            parag.add_run().add_picture("risk_bar.jpg", width=Cm(15), height=Cm(7.5))
    ...
    for parag in doc.paragraphs:
        ...
        if "weaknessCount" in row or "highRiskWeaknessCount" in row or "middleRiskWeaknessCount" in row or "lowRiskWeaknessCount" in row:
            row = row.replace("weaknessCount", str(values[0]+values[1]+values[2]), 1)
...
def high_risk_ip_list(doc, wb, sheet, ips=[]):
    ...
    for parag in doc.paragraphs:
        if "highRiskCount" in parag.text:
            parag.text = parag.text.replace("highRiskCount", str(len(tmp_ips)), 1)
            ...
            for cnt in range(idx):
                ...
                for i, title in enumerate(tag2):
                    ...
                    if i < 3:
                        ...
                        Parag.set_font(cell2.paragraphs, align=WD_ALIGN_PARAGRAPH.LEFT)
                    else:
                        ...
                        for j in range(0, 3):
                            Parag.set_font(des_table.cell(9, j).paragraphs, align=WD_ALIGN_PARAGRAPH.LEFT) 


# In[10]:


def ExcelToWord(excel, sheet, word, date, ips, save):
