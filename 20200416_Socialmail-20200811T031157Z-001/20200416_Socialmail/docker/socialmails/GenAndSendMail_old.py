...
def insert_send_mail(eml_type, eml_file, user_group, mail_excel, sm, db, annex=[]):
    # get mails from excel 
    try:
        mails = read_excel(mail_excel)
        mail_list = list(mails['Email'])
    ...
    for recipient in mail_list:
        ...
        current_html = current_html.replace('recipient_mail', recipient)
        
        # create zip annex file
        gen_annex(file_in, file_out, eml_type_id, mail_id)
...
if __name__ == '__main__':
    ...
    insert_send_mail(sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], mailserver, database, sys.argv[7])
